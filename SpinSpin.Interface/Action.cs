...
namespace SpinSpin.Interface
{
    public partial class Action : Form
    {
        ...
        public Action(int[] funkcjaPrzestrzenna, string[] funkcjaSpinowa,
            ...
            double norm, int electronNumber)
        {
            InitializeComponent();
            step = 0;
            ...
            this.norm = norm;
            this.electronsNumber = electronNumber;
        }
        ...
        private void changeStep(stepAction stepAction)
        {
            ...
            switch (step)
            {
                case 1:
                    ...
                    youngFunctions = Young.Young.FunctionOperate(baseFunction, operatorySymetryczne, operatoryAntysymetryczne, electronsNumber);
                ...
                case 2:
                    ...
                    youngSpinFunctions = Young.Young.SpinOperate(baseFunction, operatorySymetryczneSpinu, operatoryAntysymetryczneSpinu, electronsNumber);
                    stepNameLabel.Text = "Operator Younga dla funkcji spinowej";
                    showStepFunctions(youngSpinFunctions);
                    break;
                case 3:
                    completeYoung = Young.Young.FunctionsCombine(youngFunctions, youngSpinFunctions, electronsNumber);
                    antysymmetrizedFunctions = Antysymetryzator.Antysymmetrize(completeYoung, electronsNumber);
                    stepNameLabel.Text = "AN ([Y(fi)] [Y+psi]) =";
                    showStepFunctions(antysymmetrizedFunctions);
                    break;
                case 4:
                    var deltas = PermutationGenerator.GeneratePairs(electronsNumber);
                    reductionFunctionList = new List<Function>();
                    foreach (var delta in deltas)
                    {
                        var y = new List<Function>();
                        SiSjFunctions = SiSj.Operate(antysymmetrizedFunctions, delta[0], delta[1], electronsNumber);
                        var Fi = new List<BaseFunction>(); //
                        Fi.Add(baseFunction); //
                        //y = Reduktor.Redukcja(antysymmetrizedFunctions, SiSjFunctions, delta[0], delta[1], electronsNumber);
                        y = Reduktor.Redukcja(Fi, SiSjFunctions, delta[0], delta[1], electronsNumber);
                    ...
                    break;
                case 5:
                    placesChanged = PlaceChanger.ChangePlaces(reductionFunctionList, electronsNumber);
                ...
                case 6:
                    ...
                    break;
                case 7:
                    lastReduction = Reduktor.ReductionWithDelta(placesChanged, electronsNumber);
                    stepNameLabel.Text = "Final result: ";
                    foreach(var item in lastReduction)
                    {
                        item.factor = item.factor / (norm);
                    }
